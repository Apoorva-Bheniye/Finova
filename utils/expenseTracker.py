...
class ExpenseManager:
    ...
    def __init__(self,db_name):#initilization of database

        self.db_name = db_name
        self.conn = sqlite3.connect(self.db_name)#command to connect the dataset 
        self.cursor = self.conn.cursor()

        #create a table if id doesn't exist
    ...
    def addExpense(self,date,name,amount,category,description):#when user clicks on add expense button then this function is called 
        self.cursor.execute('''INSERT INTO expenses (name, date, amount, category, description)
                            VALUES(?, ?, ?, ?, ?)''',
                            (name, date, amount, category, description))#user needs to enter the details
        self.conn.commit()

    def viewExpense(self):#user can see the expense tables
        query="SELECT * FROM expenses"
        return pd.read_sql(query, self.conn)
    
    def deletExpense(self, expense_id):
...
class IncomeManager:
    ...
    def addIncome(self,date,name,amount,source,description):#when user clicks on add expense button then this function is called 
        self.cursor.execute('''INSERT INTO income (name, date, amount, source, description)
                            VALUES(?, ?, ?, ?, ?)''',
                            (name, date, amount, source, description))#user needs to enter the details
        self.conn.commit()
    
    def viewIncome(self):#user can see the income tables
        query="SELECT * FROM income"
        return pd.read_sql(query, self.conn)
    
    def deletIncome(self, income_id):
        self.cursor.execute("DELETE FROM income WHERE id=?",(income_id,))
        self.conn.commit()

#account section helps to calvulate balance and update it using both managers
class Account:
    def __init__(self, db_name):
    ...
    def deleteExpense(self,expense_id):
        ...
        if expense_id in expenses["id"].values:
            ...
            self.ExpenseManager.deletExpense(expense_id)
            self.Balance += amount
    ...
    def deleteIncome(self,income_id):
        ...
        if income_id in incomes["id"].values:
            ...
            self.IncomeManager.deletIncome(income_id)
            self.Balance -= amount
            st.success(f"Income {income_id} deleted successfully!")
        else :
            st.warning(f"Invalid Income ID: {income_id}")

#trasensation list foe ai processing
    def formate_transactions_for_ai(self):
        expenses = self.ExpenseManager.viewExpense()
