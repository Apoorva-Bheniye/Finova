...
import pandas as pd
import plotly.express as px #library installed
import os
...
if not expenses_df.empty and not income_df.empty:
    ...
    fig = px.area (
        pd.concat([monthly_expense.assign(Type="Expenses"),monthly_income.assign(Type="Income")]),
        x="month",
        y="amount",
        color="Type",
...
if not expenses_df.empty:
    ...
    fig_category_bar = px.bar(category_monthly_data,x="month", y="amount",color="category",barmode = "group",title="Monthly expenses vs Income Trend")
    st.plotly_chart(fig_category_bar)

#Stack bar chart income vs expense

if not expenses_df.empty and not income_df.empty:
    
    stacked_data = pd.concat([
        monthly_expense.assign(Type="Expenses"),
        monthly_income.assign(Type="Income")
    ])
    fig_stacked_bar = px.bar(stacked_data, x="month",y="amount",color="Type",barmode="stack",title="stacked income")
...
with st.sidebar:
    ...
    with st.expander("🗨 Chat with FinBot",expanded=False):
        st.write(f"👋 Hello {st.session_state.user_email.split('@')[0]}! How can I help you today")
        ...
        if st.button("Send 📤"):
            if user_query.strip():
                transactions_text = account.formate_transactions_for_ai()
